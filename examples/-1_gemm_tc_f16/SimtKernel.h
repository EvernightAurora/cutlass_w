...
#include <algorithm>
#include <iostream>
#include "InputIterator_deeper.h"
#include "SmemIterator.h"
#include "MmaSimt.h"
...
template<
    ...
    typename ThreadblockSwizzle_
    >
struct Gemm<TypeA_, cutlass::layout::RowMajor, TypeB_, cutlass::layout::RowMajor, TypeC_, LayoutC_, TypeAcc_,
    ...
    using WarpCount = cutlass::gemm::GemmShape<
        ...
        TBShape::kK / WShape::kK                             //1
    >;
    static int const kWarpSize = 32;//cutlass::gemm::warp::WarpSize<cutlass::arch::OpClassSimt>::value;          // 32

    template<typename PLShape, int kThreads>
    struct PLSThreadMap{
        using TensorCoord = cutlass::layout::PitchLinearCoord;

        static int const AT_1 = PLShape::kContiguous, AT_2 = PLShape::kStrided;
        static int const kElementsPerAccess = 1;
        using Iterations = cutlass::layout::PitchLinearShape<1, AT_2 / (256 / AT_1)>;
        using Delta = cutlass::layout::PitchLinearShape<1, 256 / AT_1>;
        CUTLASS_HOST_DEVICE
        static auto initial_offset(int tid){return cutlass::make_Coord(tid % AT_1, tid / AT_1);}
    };
    template<typename PLShape, int kThreads>
    struct TransposePLSThreadMap{
        static_assert(kThreads == 256);
        using TensorCoord = cutlass::layout::PitchLinearCoord;

        static int const AT_1 = PLShape::kContiguous, AT_2 = PLShape::kStrided;
        static int const kElementsPerAccess = 1;
        using Iterations = cutlass::layout::PitchLinearShape<AT_2 / (256 / AT_1), 1>;
        using Delta = cutlass::layout::PitchLinearShape<256 / AT_1, 1>;
        CUTLASS_HOST_DEVICE
        static auto initial_offset(int tid){return cutlass::make_Coord(tid / AT_1, tid % AT_1);}
    };

    
    using IteratorThreadMapA = PLSThreadMap<
        cutlass::layout::PitchLinearShape<TBShape::kK, TBShape::kM>, WarpCount::kCount * kWarpSize>;   //<8, 128> 256  ~ <<it:<1, 4>, dt:<1, 32>>>
    using SmemThreadMapA = TransposePLSThreadMap<                                                   // ~ <<it: <4, 1>, dt: <32, 1> >>
        cutlass::layout::PitchLinearShape<TBShape::kK, TBShape::kM>, 256>;      //<8, 128> 256
    
    using IteratorThreadMapB = PLSThreadMap<
        cutlass::layout::PitchLinearShape<TBShape::kN, TBShape::kK>,                                    // ~ <<it: <1, 4>,  dt:<1, 2>
        WarpCount::kCount * kWarpSize>;                                         //<128, 8> 256
    using SmemIteratorA = SMEM_ITERATOR::RegularTileIterator<                         
        ...
        SmemThreadMapA
    >;
    using SmemIteratorB = SMEM_ITERATOR::RegularTileIterator<
    ...
    static const int WarpNumThreadsN = kWarpSize / WarpNumThreadsM;                 //8
    static const int ThreadTileM = WShape::kM / WarpNumThreadsM;                    //8
    static const int ThreadTileN = WShape::kN / WarpNumThreadsN;                    //8
    static const int LaneLayout = ThreadTileM > 4 && ThreadTileN > 4 ? 2 : 1;           //2
    static const int numElementsA = 128 / cutlass::sizeof_bits<TypeA>::value;           //4
    static const int numElementsB = 128 / cutlass::sizeof_bits<TypeB>::value;           //4
    ...
    using LaneMmaShape = cutlass::gemm::GemmShape<
        ...
        LaneN,
        1>;                                                                             //<4, 4, 1>
    using Policy = cutlass::gemm::warp::MmaSimtPolicy<                          //pack 3 things
        cutlass::MatrixShape<WarpNumThreadsM, WarpNumThreadsN>,   // WarpShape  <4, 8>
        cutlass::layout::RowMajor,         // LaneLayout 
        LaneMmaShape
    >;

    using MmaWarpSimt = MMA_SIMT::MmaSimt<
    ...
    using MmaPolicy = cutlass::gemm::threadblock::MmaPolicy<
        MmaWarpSimt,
        cutlass::MatrixShape<kPaddingM, 0>,    // skew for A matrix to avoid SMEM bank conflicts        // A Padd  <4, 1>
        cutlass::MatrixShape<0, 0>,                                                                     //B Padd
        WarpCount::kK
    >;

    using IteratorA = INPUT_ITERATOR::UnpackedPTIterator<       
        cutlass::MatrixShape<TBShape::kM, TBShape::kK>,
        TypeA, LayoutA, 1, IteratorThreadMapA, 1, false>;

    using IteratorB = INPUT_ITERATOR::UnpackedPTIterator<
        cutlass::MatrixShape<TBShape::kK, TBShape::kN>,
    ...
    using RegularEpilogueShell = cutlass::epilogue::threadblock::DefaultEpilogueSimt<
        TBShape,
    ...
    struct KernelParams{
        ...
        KernelParams(
            ...
            semaphore(workspace){
                ...
                int gemm_k_iterations = (total_gemm_k_iterations + grid_tiled_shape.k() - 1) / grid_tiled_shape.k();                //p_K+K-1)/K * K
    ...
    static void ActivateKernel(const KernelParams&params, unsigned char SharedDest[]){
        ...
        int problem_size_k = min(
            params.problem_size.k(), (threadblock_tile_offset.k() + 1) * params.gemm_k_size             //P_K  < (p_k + k - 1) / k * k
        );
        ...
        typename BACK_MMA::FragmentC acc;
        acc.clear();
        if(true)
            Mma(gemm_k_iterations, acc, iter_A, iter_B, acc);
        ...
        */
        epilogue(output_op, iterator_D, acc, iterator_C);
        /*
        if(false){
            int lock;
    ...
    void settingParams(const Arguments& args, void *workspace=nullptr){
        ...
        cutlass::gemm::GemmCoord grid_shape = threadblock_swizzle.get_tiled_shape(
            args.problem_size,
            {TBShape::kM, TBShape::kN, TBShape::kK}, 1);            //(p_M + M-1)/M, (p_N+N-1)/N, 1
        params = KernelParams(args.problem_size,
            grid_shape,
