...
using G = CutlassGemm;
using TBS = CutlassGemm::DefaultType;

using MOP = CutlassGemm::ThreadblockShape;
auto mop = MOP();
using SAR = CutlassGemm::WarpShape;
auto sar = SAR();

using SW = G::ThreadblockSwizzle;
auto sw = SW();
...
void TYPE_ASSERT(){
   ...
   // std::cout<<"acc v"<<MY_GEMM::IteratorA::UnderlyingIterator::AccessType::kElements<<std::endl;
    
    //std::cout<<"Underly ptai_u: "<<MY_GEMM::IteratorA::UnderlyingIterator::TileAccessIterator::UnderlyingPredicates::SIGN_LINE<<std::endl;
...
void Process_MM(){
    ...
    MakeMatrix<TB, LayoutB>::print(pB, dpB, K, N, LayoutOutput(N)); 
*/
    ...
    if(my_status != cudaSuccess)
        std::cout<<"Failed"<<cudaGetErrorString(cudaGetLastError())<<std::endl;
    //MakeMatrix<ElementOutput, LayoutOutput>::print(pD, dpD, M, N, LayoutOutput(N));  
...
using DMma = TBS::MmaType;
using MmaCore = DMma::MmaCore;
...
int main(){
    ...
    std::cout<<"OPERATOR: "<<MY_GEMM::MmaWarpSimt::SIGN_LINE<<std::endl;

    std::cout<<"EPILOGUE: "<<MY_GEMM::RegularEpilogueShell::SIGN_LINE<<std::endl;
    std::cout<<"EPILOGUE::OutputTileThreadMap: "<<MY_GEMM::RegularEpilogueShell::OutputTileThreadMapShell::SIGN_LINE<<std::endl;
    ...
    std::cout<<"Policy::LaneMmaShape  ";SHOW_TYPE(MY_GEMM::MmaWarpSimt::Policy::LaneMmaShape);
    ...
    std::cout<<MY_GEMM::RegularEpilogueShell::OutputTileThreadMapShell::Type::Detail::kCompactedDeltaCluster<<std::endl;
    std::cout<<MY_GEMM::RegularEpilogueShell::OutputTileThreadMapShell::Type::Detail::kWarpPartitionsCluster<<std::endl;
    std::cout<<MY_GEMM::RegularEpilogueShell::OutputTileThreadMapShell::Type::Detail::kWarpsRemainingForGroups<<std::endl;
    std::cout<<MY_GEMM::RegularEpilogueShell::OutputTileThreadMapShell::Type::Detail::kIterationsGroup<<std::endl;
    ...
    //std::cout<<"SMEM_ITER_A_UNDER: "<<MY_GEMM::SmemIteratorA::Underlying::SIGN_LINE<<std::endl;
    /*

    std::cout<<"SMEM_ITER_B: "<<MY_GEMM::SmemIteratorB::SIGN_LINE<<std::endl;
    std::cout<<"SMEM_ITER_B_UNDER: "<<MY_GEMM::SmemIteratorB::Underlying::SIGN_LINE<<std::endl;
