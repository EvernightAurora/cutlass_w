...
void TYPE_ASSERT(){
    ...
    SHOW_TYPE(GUESS_MMA_CORE::IteratorThreadMapA);
...
void Init_MM(){
    MakeMatrix<TA, LayoutA>::alloc(pA, dpA, M, K, LayoutA(K), 2);
    MakeMatrix<TB, LayoutB>::alloc(pB, dpB, K, N, LayoutB(N), 2);
    MakeMatrix<TC, LayoutC>::alloc(pC, dpC, M, N, LayoutC(N), 1);
    MakeMatrix<ElementOutput, LayoutOutput>::alloc(pD, dpD, M, N, LayoutOutput(N), 1);        
}
